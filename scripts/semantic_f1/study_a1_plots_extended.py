...
def plot_metric_vs_radius_grids(
    ...
    Expects helper callables in `helpers`:
    - sample_gold_labels, to_label_names, perturb_labels
    - semantic_f1_score, f1_score, MultiLabelBinarizer
    """
    ...
    p_values = deepcopy(cfg.p_values)
    ...
    def compute_metrics_with_ci(
        ...
        """Compute metrics with bootstrap CIs, optimized to compute all metrics once per iteration."""
        rng = np.random.default_rng(seed)
        n_ex = len(T_names)
        idx_base = np.arange(n_ex)
        mlb = MultiLabelBinarizer(classes=list(S.index))
        mlb.fit([[]])

        # Import semantic functions once
        ...
        # Storage for all metric values across bootstrap iterations
        draws = {
            k: []
            for k in [
                "hard_micro",
        ...
        # Bootstrap loop: compute all metrics once per iteration
        for _ in tqdm(range(B), desc="Bootstrap iterations", leave=False):
            ...
            draws["hard_micro"].append(
                float(f1_score(Yt, Yp, average="micro", zero_division=0))
            )
            draws["hard_macro"].append(
                float(f1_score(Yt, Yp, average="macro", zero_division=0))
            )
            draws["hard_samples"].append(
                float(f1_score(Yt, Yp, average="samples", zero_division=0))
            )

            # Compute all semantic metrics (still requires 3 calls due to library API)
            samples_results = samples_semantic_f1_score(
                P_sel, T_sel, S, return_components=True
            )
            draws["sem_f1_samples"].append(float(samples_results["f1"]))
            draws["sem_precision_samples"].append(
                float(samples_results["precision"])
            )
            draws["sem_recall_samples"].append(float(samples_results["recall"]))

            micro_results = semantic_micro_f1_score(
                P_sel, T_sel, S, return_components=True
            )
            draws["sem_f1_micro"].append(float(micro_results["f1"]))
            draws["sem_precision_micro"].append(
                float(micro_results["precision"])
            )
            draws["sem_recall_micro"].append(float(micro_results["recall"]))

            macro_results = semantic_macro_f1_score(
                P_sel, T_sel, S, return_components=True
            )
            draws["sem_f1_macro"].append(float(macro_results["f1"]))
            draws["sem_precision_macro"].append(
                float(macro_results["precision"])
            )
            draws["sem_recall_macro"].append(float(macro_results["recall"]))
        ...
        for k, arr in draws.items():
            ...
            stats[k] = (mean, float(lo), float(hi))
        return stats

    # Precompute per-(k,p) per-radius stats once, reused by the three figures
    kp_stats: dict[
    ...
    mapping = {
        "hard_micro": "Hard F1",
        "hard_macro": "Hard F1",
        "hard_samples": "Hard F1",
        "sem_f1_micro": "Semantic F1",
        "sem_f1_macro": "Semantic F1",
        "sem_f1_samples": "Semantic F1",
        "sem_precision_micro": "Semantic Precision",
        "sem_precision_macro": "Semantic Precision",
        "sem_precision_samples": "Semantic Precision",
        "sem_recall_micro": "Semantic Recall",
        "sem_recall_macro": "Semantic Recall",
        "sem_recall_samples": "Semantic Recall",
    }

    for k_plot in tqdm(cfg.k_values, desc="Processing k values for plots"):
        gold_idx_cache = [
        ...
        for p_plot in tqdm(p_values, desc=f"Processing p values (k={k_plot})", leave=False):
            series_stats: dict[str, dict[int, tuple[float, float, float]]] = {}
            for r in tqdm(radii_union, desc=f"Processing radii (k={k_plot}, p={p_plot:.2f})", leave=False):
                preds_idx = [
                ...
                for mkey, (mean, lo, hi) in stats.items():
                    series = mapping[mkey]
                    series_stats.setdefault(series, {})[r] = (mean, lo, hi)
            kp_stats[(k_plot, float(p_plot))] = series_stats

    for avg_name, series_tuple in [
        (
            "micro",
            ("Hard F1", "Semantic F1", "Semantic Precision", "Semantic Recall"),
        ),
        (
            "macro",
            ("Hard F1", "Semantic F1", "Semantic Precision", "Semantic Recall"),
        ...
        series_styles = {
            "Hard F1": dict(color=HARD_COLOR, linestyle="-", marker="o"),
            "Semantic F1": dict(color=SEM_COLOR, linestyle="-", marker="s"),
            "Semantic Precision": dict(
                color=THIRD_COLOR, linestyle="--", marker="^"
            ),
            "Semantic Recall": dict(color="#E45756", linestyle=":", marker="D"),
        }
        ...
        for i, k_plot in enumerate(cfg.k_values):
            for j, p_plot in enumerate(p_values):
                ...
                for series in series_tuple:
                    ...
                    ax.plot(rs, vals, label=series, **style)
                ax.set_ylim(0.0, 1.0)
        ...
        # Column headers (top row): p=...
        for j, pval in enumerate(p_values):
            axes[0][j].set_title(f"p={pval:.2f}", fontsize=20)
...
def plot_kendall_tau_bars(
    ...
    df = df[~df["metric"].str.contains(":a=")].copy()

    # Keep only core variants for legend simplicity (including F1, precision, recall)
    allowed = {
        "hard_micro",
        "sem_f1_micro:ideal",
        "sem_f1_micro:permuted",
        "sem_precision_micro:ideal",
        "sem_precision_micro:permuted",
        "sem_recall_micro:ideal",
        "sem_recall_micro:permuted",
    }
    ...
    def _disp(m: str) -> str:
        if m == "hard_micro":
            return "Hard F1"
        if m == "sem_f1_micro:ideal":
            return "Semantic F1 (ideal)"
        if m == "sem_f1_micro:permuted":
            return "Semantic F1 (permute)"
        if m == "sem_precision_micro:ideal":
            return "Semantic Precision (ideal)"
        if m == "sem_precision_micro:permuted":
            return "Semantic Precision (permute)"
        if m == "sem_recall_micro:ideal":
            return "Semantic Recall (ideal)"
        if m == "sem_recall_micro:permuted":
            return "Semantic Recall (permute)"
        return m
...
def plot_kendall_tau_grids(
    ...
    def _keep_metric(name: str, avg: str) -> bool:
        core = {
            f"hard_{avg}",
            f"sem_f1_{avg}:ideal",
            f"sem_f1_{avg}:permuted",
            f"sem_precision_{avg}:ideal",
            f"sem_precision_{avg}:permuted",
            f"sem_recall_{avg}:ideal",
            f"sem_recall_{avg}:permuted",
        }
        return name in core
    ...
    def _variant_label(m: str, avg: str) -> str:
        if m == f"hard_{avg}":
            return "Hard F1"
        if m == f"sem_f1_{avg}:ideal":
            return "Semantic F1 (ideal)"
        if m == f"sem_f1_{avg}:permuted":
            return "Semantic F1 (permute)"
        if m == f"sem_precision_{avg}:ideal":
            return "Semantic Prec (ideal)"
        if m == f"sem_precision_{avg}:permuted":
            return "Semantic Prec (permute)"
        if m == f"sem_recall_{avg}:ideal":
            return "Semantic Rec (ideal)"
        if m == f"sem_recall_{avg}:permuted":
            return "Semantic Rec (permute)"
        return m
    ...
    for avg in ["micro", "macro", "samples"]:
        ...
        style_map: dict[str, dict[str, object]] = {
            ...
            f"sem_f1_{avg}:ideal": {"color": SEM_COLOR, "marker": "s"},
            f"sem_f1_{avg}:permuted": {"color": THIRD_COLOR, "marker": "^"},
            f"sem_precision_{avg}:ideal": {"color": "#9467BD", "marker": "D"},
            f"sem_precision_{avg}:permuted": {
                "color": "#8C564B",
                "marker": "v",
            },
            f"sem_recall_{avg}:ideal": {"color": "#E45756", "marker": "P"},
            f"sem_recall_{avg}:permuted": {"color": "#BCBD22", "marker": "X"},
        }
        ...
        # Styles and labels for grouped bars (reusing mapping from above)
        # This overwrites the style_map from above, providing label strings
        style_map_with_labels: dict[str, dict[str, object]] = {}
        for m in metrics_order:
            style_map_with_labels[m] = {
                "color": style_map[m]["color"],
                "label": _variant_label(m, avg),
            }
        ...
        for i, k_val in enumerate(ks):
            ...
            for j, m in enumerate(metrics_order):
                ...
                color = style_map_with_labels[m]["color"]
                label = style_map_with_labels[m]["label"]
                bars = ax.bar(
                    x + offsets[j],
                    heights,
                    width=bar_w,
        ...
        for m in metrics_order:
            handles.append(
                plt.Rectangle(
                    ...
                    1,
                    1,
                    facecolor=style_map_with_labels[m]["color"],
                    edgecolor="black",
                    linewidth=0.8,
                )
            )
            labels_.append(style_map_with_labels[m]["label"])
        fig.legend(
            handles,
            labels_,
            loc="lower center",
...
def plot_kendall_tau_linepanels(
    ...
    Uses negative Kendall tau, core variants only: Hard, Semantic F1/Precision/Recall (ideal, permute).
    """
    if df_tau_all.empty:
        return
...
def plot_bootstrap_heatmaps(
    ...
    if not df_ideal.empty:
        # Group metrics by type - include F1, Precision, Recall
        metric_groups = {
            "micro": [
                "hard_micro",
                "sem_f1_micro",
        ...
        for avg_type, metrics in metric_groups.items():
            ...
            # Create subplot for hard, F1, precision, recall comparison
            fig, axes = plt.subplots(1, 4, figsize=(20, 4))

            import numpy as np

            # Calculate data for all metrics to get shared scale
            pivot_data = {}
            ...
            # Get common color scale across all heatmaps
            if len(pivot_data) > 1:
                all_values = []
            ...
            metric_names = {
                "hard": "Hard F1",
                "sem_f1": "Semantic F1",
                "sem_precision": "Semantic Precision",
                "sem_recall": "Semantic Recall",
            }
            ...
            for i, metric in enumerate(metrics):
                ...
                metric_display = metric_names.get(
                    metric_type, metric_type.replace("_", " ").title()
                )

                ax.set_title(
                    f"{metric_display} ({avg_type.capitalize()})",
                    fontweight="bold",
                    pad=15,
                )
    ...
    if not df_repr.empty:
        ...
        def clean_metric_name(metric):
            ...
            elif metric == "sem_f1_micro":
                return "Semantic F1 (micro)"
            elif metric == "sem_f1_macro":
                return "Semantic F1 (macro)"
            elif metric == "sem_f1_samples":
                return "Semantic F1 (samples)"
            elif metric == "sem_precision_micro":
                return "Semantic Prec (micro)"
            elif metric == "sem_precision_macro":
                return "Semantic Prec (macro)"
            elif metric == "sem_precision_samples":
                return "Semantic Prec (samples)"
            elif metric == "sem_recall_micro":
                return "Semantic Rec (micro)"
            elif metric == "sem_recall_macro":
                return "Semantic Rec (macro)"
            elif metric == "sem_recall_samples":
                return "Semantic Rec (samples)"
            else:
                return metric
...
def plot_bootstrap_summary_charts(
    ...
    # Group by metric type - include F1, precision, recall
    metric_groups = {
        "micro": [
            "hard_micro",
            "sem_f1_micro",
    ...
    for avg_type, metrics in metric_groups.items():
        ...
        # Create 4-panel figure: Hard F1, Sem F1, Sem Precision, Sem Recall
        fig, axes = plt.subplots(1, 4, figsize=(20, 6), sharey=True)

        # Box plot of effect sizes by S
        s_order = ["ideal", "permuted"]  # Base ordering
        ...
        metric_configs = [
            {
                "filter": lambda m: m.startswith("hard"),
                "title": "Hard F1",
                "color": HARD_COLOR,
                "S_label": "Hard F1",
            },
            {
                "filter": lambda m: m.startswith("sem_f1_"),
                "title": "Semantic F1",
                "color": SEM_COLOR,
                "S_label": None,
            },
            {
                "filter": lambda m: m.startswith("sem_precision_"),
                "title": "Semantic Precision",
                "color": THIRD_COLOR,
                "S_label": None,
            },
            {
                "filter": lambda m: m.startswith("sem_recall_"),
        ...
        for idx, config in enumerate(metric_configs):
            ...
            # Filter to this metric type
            df_metric = df_metrics[
                df_metrics["metric"].apply(config["filter"])
            ].copy()

            if df_metric.empty:
                continue

            # For hard metrics, use single S category; for semantic, use actual S values
            if config["S_label"]:
                df_metric["S"] = config["S_label"]
                order = [config["S_label"]]
                palette = [config["color"]]
            else:
                order = s_order_final
                palette = [config["color"]] * len(s_order_final)
            ...
            if idx == 0:
                ax.set_ylabel("Similarity Matrix", fontsize=14)
            else:
                ax.set_ylabel("")
        ...
        fig.suptitle(
            f"Bootstrap Effect Size Distribution - {avg_type.capitalize()} Averaging",
            fontweight="bold",
            fontsize=18,
            y=0.98,
        )
    ...
    for avg in ["micro", "macro", "samples"]:
        ...
        for k_val in ks:
            ...
            for idx_m, m in enumerate(metrics_order):
                label_map = {
                    f"hard_{avg}": "Hard F1",
                    f"sem_f1_{avg}:ideal": "Sem F1 (ideal)",
                    f"sem_f1_{avg}:permuted": "Sem F1 (perm)",
                    f"sem_precision_{avg}:ideal": "Sem Prec (ideal)",
                    f"sem_precision_{avg}:permuted": "Sem Prec (perm)",
                    f"sem_recall_{avg}:ideal": "Sem Rec (ideal)",
                    f"sem_recall_{avg}:permuted": "Sem Rec (perm)",
                }

#!/usr/bin/env python3
"""
Study D Implementation: P4G vs 2-shot SemEval Comparison

This script implements Study D by comparing downstream performance on PersuasionForGood
with 2-shot SemEval performance, reading results directly from the logs directory.
...
# Emotion labels for SemEval dataset
EMOTION_LABELS = [
    'anger',
    'anticipation',
    'disgust',
...
class SemEvalProcessor:
    """Process SemEval 2-shot experiment results."""
    ...
    def load_semeval_results(
        self, metrics_path: Path
    ) -> Optional[Dict[str, float]]:
        """Load SemEval results from metrics.yml file."""
        try:
            ...
            logger.warning(
                f"Failed to load SemEval results from {metrics_path}: {e}"
            )
            return None
...
class StudyDAnalyzer:
    """Main analyzer for Study D: P4G vs 2-shot SemEval comparison."""
    ...
    def __init__(
        ...
        self.semeval_processor = SemEvalProcessor(
    ...
    def find_model_experiments(self) -> List[Tuple[str, Path, Path]]:
        ...
        # Define model name mappings between P4G and SemEval
        model_mappings = [
            # OpenAI models
            (
                'PersuasionForGoodOpenAI/gpt-4o-mini-inference-semeval-0-shot_0',
                'SemEvalOpenAI/gpt-4o-mini-inference-2-shot_0',
            ),
            (
                'PersuasionForGoodOpenAI/gpt-4.1-mini-inference-semeval-0-shot_0',
                'SemEvalOpenAI/gpt-4.1-mini-inference-2-shot_0',
            ),
            # Local models
            (
                'PersuasionForGood/meta-llama--Llama-2-7b-chat-hf-inference-semeval-0-shot_0',
                'SemEval/meta-llama--Llama-2-7b-chat-hf-inference-2-shot_0',
            ),
            (
                'PersuasionForGood/meta-llama--Llama-2-70b-chat-hf-inference-semeval-0-shot_0',
                'SemEval/meta-llama--Llama-2-70b-chat-hf-inference-2-shot_0',
            ),
            (
                'PersuasionForGood/meta-llama--Llama-3.1-8B-Instruct-inference-semeval-0-shot_0',
                'SemEval/meta-llama--Llama-3.1-8B-Instruct-inference-2-shot_0',
            ),
            (
                'PersuasionForGood/meta-llama--Llama-3.3-70B-Instruct-inference-semeval-0-shot_0',
                'SemEval/meta-llama--Llama-3.3-70B-Instruct-inference-2-shot_0',
            ),
        ]

        for p4g_path, semeval_path in model_mappings:
            p4g_full_path = self.logs_dir / p4g_path
            semeval_full_path = self.logs_dir / semeval_path

            # Check if both experiments exist
            p4g_indexed = p4g_full_path / 'indexed_metrics.yml'
            semeval_metrics = semeval_full_path / 'metrics.yml'

            model_name = p4g_path.split('/')[-1]  # Extract model directory name

            if p4g_indexed.exists() and semeval_metrics.exists():
                experiments.append((model_name, p4g_indexed, semeval_metrics))
                print(f"✓ Found complete experiment for: {model_name}")
            else:
                missing = []
                if not p4g_indexed.exists():
                    missing.append(f"P4G: {p4g_indexed}")
                if not semeval_metrics.exists():
                    missing.append(f"SemEval: {semeval_metrics}")
    ...
    def run_analysis(self, max_k: int = 10) -> Dict[str, Any]:
        ...
        logger.info("Starting Study D analysis: P4G vs 2-shot SemEval...")
        ...
        for (
            ...
            try:
                ...
                # Load SemEval results
                semeval_metrics = self.semeval_processor.load_semeval_results(
                    semeval_metrics_path
                )
                if semeval_metrics is None:
                    failed_models.append(
                        (model_name, "Failed to load SemEval results")
                    )
                    continue

                logger.info(f"Loaded {len(p4g_predictions)} P4G predictions")
                logger.info(f"Loaded {len(semeval_metrics)} SemEval metrics")
                ...
                semeval_results[model_name] = semeval_metrics
        ...
        # Calculate correlations between P4G performance and SemEval metrics
        correlations = self.calculate_correlations(results, semeval_results)

        # Save results
        output_data = {
            'results': results,
            'semeval_results': semeval_results,
        ...
        output_file = self.output_dir / 'study_d_p4g_semeval_results.pkl'
    ...
    def calculate_correlations(
        self, p4g_results: Dict[str, Any], semeval_results: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Calculate correlations between P4G downstream performance and SemEval metrics."""
        correlations = {}

        # Get all SemEval metric names
        all_semeval_metrics = set()
        for model_metrics in semeval_results.values():
            all_semeval_metrics.update(model_metrics.keys())

        # Calculate correlations for each SemEval metric vs P4G performance
        for semeval_metric in all_semeval_metrics:
            ...
            for model_name in p4g_results.keys():
                if model_name not in semeval_results:
                    continue

                # Get SemEval score
                semeval_score = semeval_results[model_name].get(semeval_metric)
                if semeval_score is None:
                    continue
                ...
                if best_p4g_score is not None:
                    ...
                    semeval_scores.append(semeval_score)
                    p4g_scores.append(best_p4g_score)

            # Calculate correlation if we have enough data points
            if len(models) >= 3:
                try:
                    corr_result = spearmanr(semeval_scores, p4g_scores)
                    correlations[semeval_metric] = {
                        'correlation': corr_result.correlation,
                        'p_value': corr_result.pvalue,
                        'n_models': len(models),
                        'data': {
                            'models': models,
                            'semeval_scores': semeval_scores,
                            'p4g_scores': p4g_scores,
                        },
                    }
                except Exception as e:
                    logger.warning(
                        f"Failed to calculate correlation for {semeval_metric}: {e}"
                    )
...
def main():
    parser = argparse.ArgumentParser(
        description='Study D: Compare P4G performance with 2-shot SemEval'
    )
    parser.add_argument(
        '--logs-dir',
        type=str,
    ...
    print(
        f"\nResults saved to: {args.output_dir}/study_d_p4g_semeval_results.pkl"
    )

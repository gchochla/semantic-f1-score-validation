...
def plot_metric_vs_radius_grids(
    ...
    mapping = {
        ...
        "sem_micro": "Semantic F1",
        "sem_macro": "Semantic F1",
        "sem_samples": "Semantic F1",
    }
    ...
    for avg_name, series_pair in [
        ("micro", ("Hard F1", "Semantic F1")),
        ("macro", ("Hard F1", "Semantic F1")),
        ("samples", ("Hard F1", "Semantic F1")),
    ]:
        series_styles = {
            series_pair[0]: dict(color=HARD_COLOR, linestyle="-", marker="o"),
            series_pair[1]: dict(color=SEM_COLOR, linestyle="-", marker="s"),
        }
        ...
        for j, pval in enumerate(p_values):
            axes[0][j].set_title(f"p={pval:.2f}", fontsize=24)
        ...
        fig.supxlabel("Hop Radius r", fontsize=24, y=0.05)
...
def plot_kendall_tau_bars(
    ...
    allowed = {"hard_micro", "sem_micro:ideal", "sem_micro:permuted"}
...
def plot_kendall_tau_grids(
    ...
    def _keep_metric(name: str, avg: str) -> bool:
        core = {f"hard_{avg}", f"sem_{avg}:ideal", f"sem_{avg}:permuted"}
        return name in core
    ...
    for avg in ["micro", "macro", "samples"]:
        ...
        style_map: dict[str, dict[str, object]] = {
            f"hard_{avg}": {"color": HARD_COLOR, "marker": "o"},  # blue
            f"sem_{avg}:ideal": {"color": SEM_COLOR, "marker": "s"},  # orange
            f"sem_{avg}:permuted": {
                "color": THIRD_COLOR,
                "marker": "^",
            },  # green
...
def plot_kendall_tau_linepanels(
    ...
    Uses negative Kendall tau, core variants only: Hard, Semantic (ideal), Semantic (permute).
    """
    if df_tau_all.empty:
        return
    ...
    for avg in ["micro", "macro", "samples"]:
        ...
        order = [f"hard_{avg}", f"sem_{avg}:ideal", f"sem_{avg}:permuted"]
...
def plot_bootstrap_summary_charts(
    ...
    for avg_type, metrics in metric_groups.items():
        ...
        sns.boxplot(
            ...
            ax=ax,
            palette=[SEM_COLOR] * len(s_order_final)
            + [HARD_COLOR],  # Semantic F1 in blue, Hard F1 in orange
            legend=False,
        )
        ...
        legend_elements = [
            Patch(facecolor=SEM_COLOR, label='Semantic'),
            Patch(facecolor=HARD_COLOR, label='Hard'),
        ]
        ax.legend(
